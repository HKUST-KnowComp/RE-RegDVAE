...
import nltk
import re, string
import definitions.settings
import pickle

"""
info format:
0 lexicalized dependency path between arguments (entities) of the relation,
1 entity types of the first and second entity
2 trigger word
3 raw sentence
4 pos tags of the entire sentence
5 id of the sentence
"""
parsing = 0
entities = 1
trig = 2
...
def bow(info, arg1, arg2) -> list:
    sentence_str = info[sentence]  # type: str
    arg1idx = sentence_str.find(arg1)  # type: int
    arg2idx = sentence_str.rfind(arg2) + len(arg2)
    return sentence_str[arg1idx:arg2idx].split()
    # return info[sentence][info[sentence].find(arg1):info[sentence].rfind(arg2)+len(arg2)].split()


def bow_clean(info, arg1, arg2) -> list:
    bow_raw = bow(info, arg1,
                  arg2)  # info[sentence][info[sentence].find(arg1):info[sentence].rfind(arg2)+len(arg2)].split()
    result = []
    tmp = []
    for word in bow_raw:
...
def trigger(info, arg1, arg2):
    # trigger format: "TRIGGER:production"
